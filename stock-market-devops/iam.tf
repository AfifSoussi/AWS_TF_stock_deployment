...
# Attach S3 full access to ECS task role for accessing the S3 bucket
resource "aws_iam_role_policy_attachment" "task_s3" {
  role       = aws_iam_role.ecs_task_role.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonS3FullAccess"
}

# Attach ECS Task Execution Policy (Already existing)
resource "aws_iam_role_policy_attachment" "ecs-task-execution-role-policy-attachment" {
  role       = aws_iam_role.ecs_task_execution_role.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy"
}

# IAM Policy Document for Autoscaling 
...
# Add IAM Access Key for ECS task (if your ECS tasks need AWS credentials to access S3)
